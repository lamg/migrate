...
Composite primary keys are supported. They can be declared either in one attribute or by repeating `PK`:
...
```sql
CREATE TABLE account(tenant_id TEXT NOT NULL, user_id TEXT NOT NULL, name TEXT NOT NULL, PRIMARY KEY(tenant_id, user_id));

CREATE TABLE product(tenant_id TEXT NOT NULL, sku TEXT NOT NULL, name TEXT NOT NULL, PRIMARY KEY(tenant_id, sku));
```
...
If the base type has a composite primary key, the generated extension table uses the corresponding composite foreign key and primary key columns.
...
If the referenced type has a composite primary key, the referencing field expands to multiple foreign-key columns. Single-column references keep the existing `_id` naming. Composite references use `<field>_<pkColumn>` for each PK part:
...
CREATE TABLE wallet(id INTEGER PRIMARY KEY AUTOINCREMENT, user_tenant_id TEXT NOT NULL, user_user_id TEXT NOT NULL, address TEXT NOT NULL, FOREIGN KEY (user_tenant_id, user_user_id) REFERENCES user(tenant_id, user_id));
```
...
For composite references, `OnDeleteCascade` and `OnDeleteSetNull` still target the relationship field name rather than the generated FK column names:
...
This produces `ON DELETE CASCADE` on the composite foreign key `(user_tenant_id, user_user_id)`.
...
Join inference also supports composite foreign keys. A composite relationship generates one equality per FK/PK pair joined with `AND`.
...
The supported CLI code-generation path starts from the runtime project directory plus the compiled `MigSchema` project.
...
The runtime directory must contain exactly one runtime `.fsproj`. The schema project must live at `MigSchema/MigSchema.fsproj`. By convention, the first schema source file is `MigSchema/Main.fs`, but codegen only requires compiled attributed modules. Schema types may be split across multiple compiled modules, and every schema module must be marked with the same non-empty `GeneratedDbNamespaceAttribute` value.

`mig codegen` writes `Db.fs` into the schema directory. The generated module contains:
