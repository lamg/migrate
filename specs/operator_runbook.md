...
- Target: new SQLite database for the current schema (`<dir-name>-<schema-hash>.sqlite`)
...
2. Confirm the current schema input is ready:
   - `Schema.fs` exists in the target directory for schema-hash/path inference
   - the generated `Db` module can be loaded from the target assembly
...
```sh
mig plan [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```

## Phase 1: Migrate

```sh
mig migrate [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```

Default path inference:

- uses `./Schema.fs` for schema-bound target-path inference
...
- runtime commands still require `--assembly` / `--module` so `mig` can load the generated `Db` module
...
```sh
mig status [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```
...
Configure the service with the explicit schema-bound filename generated by `mig codegen` (`<Module>.DbFile`) rather than any wildcard/template path.

## Phase 2: Drain

```sh
mig drain [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```
...
```sh
mig cutover [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```
...
```sh
mig archive-old [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db
```
...
```sh
mig reset [--dir|-d /path/to/project] --assembly /path/to/App.dll --module Db [--dry-run]
```
...
1. `mig reset --dry-run --assembly /path/to/App.dll --module Db` to inspect inferred impact and confirm the command is not blocked.
2. `mig reset --assembly /path/to/App.dll --module Db` to apply cleanup when the dry run reports `Reset can be applied: yes`.
