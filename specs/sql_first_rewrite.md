...
1. **SQL migrations (DbUp)** are the schema source of truth.
...
4. **No** F#-first schema DSL, normalization, SqlProvider, or declarative hot-migrate pipeline.
...
mig codegen  (CLI and MigLib public API)
  1. Collect migration scripts (ordered)
...
MigLib
  ...
  - Public DbUp migrate helper: Task<Result<...>>
```

Package layout stays: **`migtool` CLI + `MigLib` library** (+ `MigLib.Web` only if still relevant; out of core rewrite).

## Source of truth split

| Concern | Owner |
|--------|--------|
| DDL, seeds, backfills | DbUp SQL scripts in the app |
| Applied-script journal | DbUp (`SchemaVersions` or equivalent) |
...
**DbUp migrate (public, for app startup and `build.fsx`):**
...
// Optional file/directory variant if useful for scripts
val migrateScripts:
  connectionString: string ->
...
Logging defaults quiet; errors as `Result`.
...
- Tests: annotation association, introspection, codegen snapshots, runtime helpers, DbUp helper, view write-op rejection.
- Dogfood: marketbot.
...
- In-memory vs temp file for the codegen DB (pick whatever DbUp + connection lifetime needs).
...
- CLI migrate/status out of scope; library DbUp helper yes.
