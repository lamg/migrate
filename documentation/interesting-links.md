# Interesting links

References that may be useful for further development of migrate (schema design, SQLite operations, related theory).

## Schema and migrations

- [Declarative migrations for SQLite](https://david.rothlis.net/declarative-schema-migration-for-sqlite/)
- [Acadia, Rethinking Database Programming](https://acadia.engineering/blog/rethinking-database-programming)

## SQLite concurrency and production

- [Four different ways to handle SQLite concurrency](https://medium.com/@gwendal.roue/four-different-ways-to-handle-sqlite-concurrency-db3bcc74d00e)
- [Concurrency Control and Database Recovery in SQLite](https://dev.to/lovestaco/concurrency-control-and-database-recovery-in-sqlite-2pmo)
- [SQLite in Production: Optimizing WAL Mode, Concurrency, and VFS Layers for Low-Latency App Servers](https://micrologics.org/blog/sqlite-in-production-optimizing-wal-mode-concurrency-and-vfs-layers-for-low-latency-app-servers)

## Theory / data modeling

- [Categorical Databases](https://categoricaldata.net/)
- [What Category Theory teaches us about data frames](https://mchav.github.io/what-category-theory-teaches-us-about-dataframes/)
