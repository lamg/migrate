module internal MigLib.Codegen.NormalizedQueryGeneratorUpdateDelete
...
let private generateUpdateBaseSql (baseTable: CreateTable) : string =
  ...
  let pkColumnNames = pkColumns |> List.map (fun col -> col.name) |> Set.ofList
