module internal MigLib.Codegen.NormalizedQueryGeneratorInsertSelect
...
let private extensionInsertClause (baseTable: CreateTable) (extension: ExtensionTable) (typeName: string) =
  ...
  let allInsertColumns =
    extension.fkColumns @ (extensionInsertColumns |> List.map (fun col -> col.name))

  let extensionColumnNames = allInsertColumns |> String.concat ", "

  let extensionParamNames =
    allInsertColumns |> List.map (fun col -> $"@{col}") |> String.concat ", "
  ...
  let extensionBindings =
    ...
    | None ->
      let fkBindings =
        ...
        ||> List.map2 (fun fkColumn pkColumn ->
          $"cmd.Parameters.AddWithValue(\"@{fkColumn}\", {getColumnVarName pkColumn}) |> ignore")
