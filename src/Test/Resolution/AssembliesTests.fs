...
let ``resolveSchemaAssembly uses MigSchema project file name when AssemblyName is absent`` () =
  ...
  try
    ...
    | Ok assembly ->
      ...
      Assert.Equal(Path.GetFullPath expectedAssemblyPath, assembly.assemblyPath)
    | Error error -> failwith $"Expected MigSchema assembly to resolve, got: {error}"
  finally
    Directory.Delete(tempDir, true)
...
let ``resolveSchemaAssembly uses AssemblyName when present`` () =
  ...
  try
    ...
    writeFile
      ...
      "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><AssemblyName>Custom.MigSchema</AssemblyName></PropertyGroup></Project>"

    let expectedAssemblyPath = schemaDllPath tempDir "net10.0" "Custom.MigSchema"

    writeFile expectedAssemblyPath ""

    match resolveSchemaAssembly project with
    | Ok assembly ->
      Assert.Equal("Custom.MigSchema", assembly.assemblyName)
      Assert.Equal(Path.GetFullPath expectedAssemblyPath, assembly.assemblyPath)
    | Error error -> failwith $"Expected MigSchema assembly to resolve, got: {error}"
  finally
    Directory.Delete(tempDir, true)
...
let ``resolveSchemaAssembly returns regular error when project file is missing`` () =
  ...
  try
    ...
    |> assertRegularErrorContains "MigSchema project file was not found"
  finally
    Directory.Delete(tempDir, true)
...
let ``resolveSchemaAssembly returns regular error when build output is missing`` () =
  ...
  try
    ...
    result |> assertRegularErrorContains "Could not resolve MigSchema assembly"
    result |> assertRegularErrorContains "Expected build output"
    result |> assertRegularErrorContains "Build the MigSchema project first"
  finally
    Directory.Delete(tempDir, true)
...
let ``resolveSchemaAssembly trims project property values`` () =
  ...
  try
    ...
    writeFile
      project.schemaProjectPath
      "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><AssemblyName>  Custom.MigSchema  </AssemblyName><TargetFramework>  net9.0  </TargetFramework></PropertyGroup></Project>"

    let expectedAssemblyPath = schemaDllPath tempDir "net9.0" "Custom.MigSchema"
