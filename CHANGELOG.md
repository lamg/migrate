...
- **MigLib.Codegen / `mig codegen`**: filter kind `in` (`-- mig:filter name in column`).
  Emits `T list option`. `Some []` becomes `1=0`; non-empty becomes `[col] IN (@p_0, …)`.
...
- **mig CLI / package versions**: MigLib, MigLib.Codegen, and migtool aligned at `12.0.0`.
