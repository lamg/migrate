...
module K = Migrate.SqlParser.Keyword
module S = Migrate.SqlParser.Symbol
...
let alias () =
  ...
  let p = Migrate.SqlParser.Select.aliasTable pzero
  let testName = "Alias"
...
let joinOp () =
  ...
  let p = Migrate.SqlParser.Select.joinOp pzero
  let testName = "JoinOp"
  cases |> List.iteri (fun i -> parseStatementTest $"{testName}-{i}" p)
...
let tableExpr () =
  ...
  let p = Migrate.SqlParser.Select.tableExpr pzero
...
let fromExpr () =
  ...
  let p = Migrate.SqlParser.Select.fromExpr pzero
  cases |> List.iteri (fun i -> parseStatementTest $"{testName}-{i}" p)
...
let selectExpr () =
  ...
  let p = Migrate.SqlParser.Select.selectQuery pzero |>> (fun x -> x.select)
  cases |> List.iteri (fun i -> parseStatementTest $"{testName}-{i}" p)
...
let notExists () =
  ...
  cases
  |> List.iteri (fun i ->
    parseStatementTest $"NotExists-{i}" (Migrate.SqlParser.Scalar.scalarOp Migrate.SqlParser.Select.withSelect))
...
let notInSelect () =
  ...
  cases
  |> List.iteri (fun i ->
    parseStatementTest $"notInSelect-{i}" (Migrate.SqlParser.Scalar.scalarOp Migrate.SqlParser.Select.withSelect))
...
let selectWith () =
  ...
  cases
  |> List.iteri (fun i -> parseStatementTest $"SelectWith-{i}" Migrate.SqlParser.Select.withSelect)
...
let funcCoalesce () =
  ...
  cases
  |> List.iteri (fun i ->
    parseStatementTest $"funcCoalesce-{i}" (Migrate.SqlParser.Scalar.columnOrFunc Migrate.SqlParser.Select.withSelect))
...
let unionSelect () =
  ...
  cases
  |> List.iteri (fun i -> parseStatementTest $"unionSelect-{i}" Migrate.SqlParser.Select.unionSelect)
